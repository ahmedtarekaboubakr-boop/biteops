...
function Inventory() {
    ...
    try {
      if (activeSection === 'transfers') {
        const [outgoing, incoming] = await Promise.all([
          ...
          axios.get('/api/inventory/transfers/incoming')
        ])
        setTransfers(outgoing.data)
        setIncomingTransfers(incoming.data)
      } else if (activeSection === 'waste') {
        const response = await axios.get('/api/inventory/waste')
  ...
  const handleFormSubmit = async (data) => {
    try {
      if (formType === 'receive') {
        await axios.post('/api/inventory/receive', data)
      } else if (formType === 'add') {
        await axios.post('/api/inventory', data)
      } else if (formType === 'transfer') {
        await axios.post('/api/inventory/transfer', data)
      } else if (formType === 'waste' || formType === 'dispose') {
        await axios.post('/api/inventory/waste', { ...data, type: formType })
      }
      setShowForm(false)
      fetchData()
    } catch (error) {
...
function SpotCheckForm({ checkType, date, spotChecks, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true)
    try {
      await axios.post('/api/inventory/spot-check', {
        date,
        checkType,
