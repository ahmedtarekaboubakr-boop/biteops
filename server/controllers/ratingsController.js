...
export async function getRatings(req, res) {
  try {
    ...
    res.json(filtered.map(r => ({
      ...r,
      id: r._id.toString(),
      staff_id: r.staff_id?._id?.toString() ?? r.staff_id?.toString(),
...
export async function createRating(req, res) {
  try {
    ...
    if (!staffId || !date) {
      return res.status(400).json({ error: 'Missing required fields: staffId and date' });
    }
    if (performance !== null && performance !== undefined && (performance < 0 || performance > 5)) {
    ...
    const hygieneGroomingScore =
      ...
      (blackPants ? 1 : 0) +
      (correctFootwear ? 1 : 0);
    const totalScore = hygieneGroomingScore + (performance ?? 0);

    // Base update — always write hygiene fields
    const ratingData = {
    ...
    // Only update performance if a value was actually provided
    if (performance !== null && performance !== undefined) {
      ratingData.performance = performance;
    }
    ...
    if (req.user.role === 'manager') {
      ...
      const existing = await Rating.findOne({ staff_id: staffId, date, manager_id: req.user.id });
      if (existing) {
        // Preserve existing performance if no new performance was provided
        const finalPerf = ratingData.performance !== undefined ? ratingData.performance : (existing.performance ?? 0);
        const finalScore = hygieneGroomingScore + (finalPerf ?? 0);
        await Rating.findByIdAndUpdate(existing._id, { ...ratingData, performance: finalPerf, total_score: finalScore, updated_at: new Date() });
        res.json({ message: 'Rating updated successfully', id: existing.id });
      } else {
        const rating = await Rating.create({
          staff_id: staffId, date, manager_id: req.user.id, branch: manager.branch,
          ...ratingData,
      ...
      const staff = await User.findOne({ _id: staffId, role: 'staff', $or: [{ status: 'active' }, { status: null }] }).select('branch');
      ...
      if (existing) {
        ...
        const rating = await Rating.create({
          staff_id: staffId, date, manager_id: req.user.id, branch: staff.branch,
          ...ratingData,
          performance: performance ?? null,
          total_score: totalScore
        });
